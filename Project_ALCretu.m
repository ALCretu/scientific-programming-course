...
try
    ...
    %% Save some vectors which do not change during the experiment
    ...
    DrawFormattedText( win, '< Press any key to continue >','center', 950, [100, 130, 150]);
    ...
    while 1
        ...
        while 1
            ...
            if TrialState == 1 && (GetSecs()-TrialStartTime>1)%
                startnewTrials=GetSecs();
                TrialState = 2;
                break;
            elseif TrialState == 2 && (GetSecs()-startnewTrials>jitterCue(ii))  %%
                CueTime = GetSecs();
                TrialState = 3;
            elseif TrialState == 3  && GetSecs() - CueTime >3 %response==true  %%
                ResponseStartTime = GetSecs();
                TrialState = 4;
            elseif TrialState == 4 && (GetSecs()-ResponseStartTime>jitterBreak(ii))%
                startnewTrials=GetSecs();
                TrialState = 2;
                break;
            elseif TrialState==5 && (GetSecs()-experimentStartTime >200) %% like this all experiments have the same duration
                exitExperiment = true;
                break;
            end
    ...
    % toc
    disp(['experiment measured: ' num2str(expEndTime - experimentStartTime)]);
    Data.TotalDuration=expEndTime - experimentStartTime;
    KbQueueRelease;
    sca;
    % clean up
    ShowCursor;
